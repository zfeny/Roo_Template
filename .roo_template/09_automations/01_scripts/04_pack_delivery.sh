...
WO="$1"
DP=".roo_template/07_delivery_packs/${WO}"
...
- Quality Report: \`.roo_template/06_quality/04_reports/${WO}/01_quality_report.md\`
- Commands: \`.roo_template/06_quality/04_reports/${WO}/02_commands.md\`
EOT
...
bash .roo_template/09_automations/01_scripts/05_validate_wo.sh "${WO}" delivery

if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  CUR_BRANCH="$(git rev-parse --abbrev-ref HEAD)"
  if [[ "${CUR_BRANCH}" =~ ^wo/${WO}-[a-z0-9._-]+$ ]] && git show-ref --verify --quiet refs/heads/main; then
    bash .roo_template/09_automations/01_scripts/07_prepare_review_artifacts.sh "${WO}" "${CUR_BRANCH}" "${ARTIFACT_MODE}"
  fi
fi
