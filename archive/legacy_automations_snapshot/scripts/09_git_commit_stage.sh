...
add_stage_files() {
  case "${STAGE}" in
    work-order)
      git add ".roo_process/work_orders/${WO}"
      ;;
    context)
      git add ".roo_process/context_packs/${WO}"
      ;;
    implementation)
      # Keep implementation scope explicit and auditable.
      git add src .roo_process/srv .roo_process/quality/01_tests .roo_process/quality/02_lint .roo_process/quality/03_security .roo_process/automations .roo_process/docs .roo .roomodes 2>/dev/null || true
      ;;
    quality)
      bash .roo_process/automations/scripts/05_validate_wo.sh "${WO}" quality
      git add ".roo_process/quality/04_reports/${WO}"
      ;;
    delivery)
      bash .roo_process/automations/scripts/04_pack_delivery.sh "${WO}"
      git add ".roo_process/evidence/${WO}" ".roo_process/quality/04_reports/${WO}/validation_delivery.md" 2>/dev/null || true
      ;;
    review)
      git add ".roo_process/review_reports/${WO}"
      ;;
    *)
