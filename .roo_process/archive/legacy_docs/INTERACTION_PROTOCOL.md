...
* `.roo_process/context_packs/<WO_ID>/01_ContextPack.md` 等
* `.roo_process/evidence/<WO_ID>/01_DeliveryPack.md` 等
...
* 设计说明可落在 `.roo_process/docs/` 或 `.roo_process/context_packs/` 的附加文件（由资料员归档到 Delivery Pack 中引用）。
...
* `src/` 代码改动
* （如要求）`.roo_process/quality/01_tests/` 或其他测试内容
...
* `.roo_process/review_reports/<WO_ID>/01_Review.md`（包含 Verdict: PASS/FAIL + Blocking/Non-blocking）
...
* `.roo_process/quality/04_reports/<WO_ID>/01_quality_report.md`（事实报告）
...
* `.roo_process/_SPECs/01_SPEC.md`
* `.roo_process/_SPECs/02_ACCEPTANCE.md`
...
* 工单以目录为权威：`.roo_process/work_orders/<WO_ID>/`
...
1. 总包下发工单（创建 `.roo_process/work_orders/...`）并创建施工分支 `wo/<WO_ID>-...`
2. 资料员准备 Context Pack（`.roo_process/context_packs/...`）
3. Architect/Coder 轮换施工（同一上下文内切模式，不新开子任务）
4. （可选）质检员跑验证并输出报告（`.roo_process/quality/...`）
5. 资料员归档固化，组装 Delivery Pack（`.roo_process/evidence/...`）
...
2. 监理只读材料：
   ...
   * `diff` 或 `patch`（可存放在 `.roo_process/evidence/<WO_ID>/99_artifacts/`）
   * （可选）质检摘要引用
3. 监理输出 `.roo_process/review_reports/<WO_ID>/01_Review.md`，给出 PASS/FAIL
...
`.roo_process/evidence/<WO_ID>/` 最少包含：
...
* 只有当 `.roo_process/review_reports/<WO_ID>/01_Review.md` 包含 `Verdict: PASS` 才允许合并。
* 推荐 `--no-ff` 合并保留审计节点。
...
* **新增文件：** `.roo_process/docs/05_interaction_protocol.md`
* 并在 `.roo/rules/02_repo_workflow.md` 中添加引用：

  * “交互模式与职责边界详见 `.roo_process/docs/05_interaction_protocol.md`”。
...
* [ ] `.roo_process/work_orders/<WO_ID>/01_WorkOrder.md` 存在
* [ ] `.roo_process/context_packs/<WO_ID>/01_ContextPack.md` 存在
* [ ] `.roo_process/evidence/<WO_ID>/01_DeliveryPack.md` 存在
* [ ] `.roo_process/review_reports/<WO_ID>/01_Review.md` 存在且 Verdict=PASS
* [ ] （如适用）`.roo_process/quality/04_reports/<WO_ID>/01_quality_report.md` 已被 `04_Verification.md` 引用
...
1. 创建 `.roo_process/docs/05_interaction_protocol.md`，内容即本文。
