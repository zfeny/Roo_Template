...
merge_check() {
  local review="08_review_reports/${WO}/01_Review.md"
  local delivery="07_delivery_packs/${WO}/01_DeliveryPack.md"
  ...
  if rg -n '^\s*Verdict:\s*PASS\s*$|^\s*-\s*Decision:\s*PASS\s*$' "${review}" >/dev/null 2>&1; then
